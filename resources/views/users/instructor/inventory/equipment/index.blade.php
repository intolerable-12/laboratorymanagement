...
@php
    ...
    $sortUrl = function (string $column) use ($sortQuery, $currentSort, $currentDirection) {
        ...
        return route('instructor.inventory.equipment.index', array_merge($sortQuery, [
            'sort' => $column,
...
@section('content')
    <div class="account-page inventory-page">
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-4">
            @include('users.instructor.inventory.partials.view-switcher', ['active' => 'equipment'])
        </div>
            ...
            <div class="col-12 col-sm-6">
                <div class="card metric-card h-100">
                    <div class="card-body">
                        ...
                        <div class="small text-secondary">Equipment groups available to browse</div>
                    </div>
                </div>
            </div>
            <div class="col-12 col-sm-6">
                <div class="card metric-card h-100">
                    <div class="card-body">
                        <div class="small text-uppercase text-secondary mb-2">Available equipment</div>
        ...
        <div class="section-card mb-4">
            <div class="card-body p-3 p-xl-4">
                <form method="GET" action="{{ route('instructor.inventory.equipment.index') }}" class="row g-3 align-items-end" data-live-search-form="instructor-equipment-categories">
                    <div class="col-12 col-lg-8">
                        <label for="equipment-category-search" class="form-label fw-medium mb-1">Search categories</label>
                        <input type="search" id="equipment-category-search" name="search" value="{{ $search }}" placeholder="Category name, code, or description" class="form-control admin-form-control">
                    </div>
                    <div class="col-12 col-lg-auto d-flex gap-2">
                        <input type="hidden" name="sort" value="{{ $currentSort }}">
                        <input type="hidden" name="direction" value="{{ $currentDirection }}">
                        <button type="submit" class="btn btn-primary px-4">Search</button>
                        <a href="{{ route('instructor.inventory.equipment.index') }}" class="btn btn-outline-secondary px-4">Clear</a>
                    </div>
                </form>
            </div>
        </div>

        <div data-live-search-results="instructor-equipment-categories">
            <div class="section-card" id="equipmentCategoriesTable">
                <div class="card-header bg-white border-0 pt-4 px-4 px-xl-5">
                    <h3 class="h5 fw-semibold mb-3">Equipment categories</h3>
                </div>
                                    ...
                                    <tr>
                                        <td class="ps-4">
                                            <div class="d-flex align-items-center gap-3">
                                                ...
                                                <div>
                                                    <a href="{{ route('instructor.inventory.equipment.categories.show', $category) }}" class="fw-semibold text-dark text-decoration-none">{{ $category->category_name }}</a>
                                                    <div class="small text-secondary">{{ $category->category_code ?: 'Equipment category' }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td><div class="text-secondary">{{ $category->description ?: 'Available equipment in this category.' }}</div></td>
                                        <td><span class="badge text-bg-{{ $category->available_equipment_count === 0 ? 'secondary' : 'success' }}">{{ $category->available_equipment_count }} item{{ $category->available_equipment_count === 1 ? '' : 's' }}</span></td>
                                        <td class="text-center pe-4">
                                            <a href="{{ route('instructor.inventory.equipment.categories.show', $category) }}" class="btn btn-sm btn-outline-primary"><i class="fa-solid fa-eye me-1" aria-hidden="true"></i></a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="4" class="text-center text-secondary py-5">No equipment categories are available right now.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
