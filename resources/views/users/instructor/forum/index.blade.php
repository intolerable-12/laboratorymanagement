    ...
    <div class="account-page">
        <section class="hero-banner social-hero card border-0 mb-4">
            <div class="card-body p-4 p-xl-5 d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-4">
                <div class="hero-copy">
                    <div class="social-eyebrow mb-3">Instructor forum</div>
                ...
                <a href="{{ route('instructor.forum.create') }}" class="btn btn-primary px-4 rounded-pill">New post</a>
            </div>
        </section>
            ...
            <div class="col-xl-8">
                <div class="card social-card border-0 mb-4">
                    <div class="card-body p-4 p-xl-5">
                        <form method="GET" action="{{ route('instructor.forum.index') }}" class="vstack gap-3">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                <a href="{{ route('instructor.forum.index', array_filter(['search' => $search], fn ($value) => $value !== null && $value !== '')) }}" class="btn btn-sm rounded-pill {{ $category === '' ? 'btn-primary' : 'btn-outline-secondary' }}">All</a>
                                @foreach ($categories as $option)
                                    <a href="{{ route('instructor.forum.index', array_filter(['search' => $search, 'category' => $option], fn ($value) => $value !== null && $value !== '')) }}" class="btn btn-sm rounded-pill {{ $category === $option ? 'btn-primary' : 'btn-outline-secondary' }}">{{ $option }}</a>
                                @endforeach
                            </div>
                ...
                @forelse ($forumPosts as $forumPost)
                    @php
                        $authorName = trim(($forumPost->user?->first_name ?? '') . ' ' . ($forumPost->user?->last_name ?? '')) ?: 'Instructor';
                            ...
                            <div class="d-flex gap-3">
                                ...
                                <div class="flex-grow-1 min-w-0">
                                    ...
                                    <div class="d-flex flex-column flex-lg-row justify-content-between gap-3">
                                        ...
                                        <a href="{{ route('instructor.forum.show', $forumPost) }}" class="btn btn-outline-primary rounded-pill align-self-start">Open post</a>
                                    </div>
                ...
                @empty
                    <div class="card social-card border-0">
                        <div class="card-body p-5 text-center">
                            ...
                            <a href="{{ route('instructor.forum.create') }}" class="btn btn-primary rounded-pill px-4">Create the first post</a>
                        </div>
                    </div>
                @endforelse
