...
<div class="forum-comment-card" style="margin-left: {{ $level * 1.25 }}rem;">
    <div class="d-flex justify-content-between align-items-start gap-3">
        <div class="d-flex align-items-center gap-3">
            <div class="comment-avatar">{{ strtoupper(mb_substr(trim(($comment->user?->first_name ?? 'I') . ' ' . ($comment->user?->last_name ?? '')), 0, 1)) }}</div>
            <div>
    ...
    @if (! $forumPost->is_locked)
        <form method="POST" action="{{ route('instructor.forum.comments.store', $forumPost) }}" class="mt-3 vstack gap-2">
            @csrf
    ...
    @foreach ($children as $child)
        @include('users.instructor.forum.partials.comment', ['comment' => $child, 'forumPost' => $forumPost, 'level' => $level + 1])
    @endforeach
</div>
