<div data-reservation-tab-content="chemical">
    @if ($selectedLaboratoryId)
        <div class="row g-3 align-items-end mb-4">
            ...
            <div class="col-md-6">
                <label class="form-label small fw-semibold text-dark mb-1" for="reservation-chemical-search">Search chemicals</label>
                <div class="input-group">
                    <span class="input-group-text bg-white"><i class="fa-solid fa-magnifying-glass text-secondary" aria-hidden="true"></i></span>
                    <input type="search" id="reservation-chemical-search" class="form-control" data-reservation-item-search placeholder="Name, code, or barcode" value="{{ request()->query('search', '') }}" autocomplete="off">
                </div>
            </div>
        ...
        <div class="d-flex justify-content-between align-items-center gap-3 mt-3 flex-wrap" data-reservation-pagination>
            ...
            <div>
                {{ $chemicalItems->appends(['tab' => 'chemical', 'fragment' => 'chemical', 'laboratory_id' => $selectedLaboratoryId, 'search' => request()->query('search')])->links('pagination::bootstrap-5') }}
            </div>
        </div>
    @else
        <div class="alert alert-light border rounded-4 mb-0">Select a laboratory to view its available chemicals.</div>
    @endif
</div>
