        ...
        <section class="hero-banner card border-0 mb-4">
            <div class="card-body p-4 p-xl-5 d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">
                <div>
                    ...
                    <p class="mb-0 text-secondary">Check the requested laboratory items and decide whether to approve or reject the request.</p>
                </div>
                <a href="{{ route('facilitator.reservations.index') }}" class="btn btn-outline-secondary px-4">Back to Queue</a>
            </div>
        </section>
                                                        ...
                                                        @forelse ($reservation->items as $item)
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td style="max-width: 150px;">
                                                                    <input type="number" step="{{ $quantityStep }}" min="{{ $quantityMin }}" name="items[{{ $item->id }}][quantity]" value="{{ old('items.' . $item->id . '.quantity', $item->quantity) }}" class="form-control text-end @error('items.' . $item->id . '.quantity') is-invalid @enderror">
                                                                    @error('items.' . $item->id . '.quantity')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                                                                </td>
                                                            </tr>
                                                        @empty
                                                            <tr><td colspan="2" class="text-center text-secondary py-3">No items were attached to this reservation.</td></tr>
                                                        @endforelse
                                                    </tbody>
                                                </table>
                                            </div>
