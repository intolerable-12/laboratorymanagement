    ...
    <div style="max-width:720px; margin:0 auto; padding:32px 16px;">
        <div style="background:#ffffff; border:1px solid #e5e7eb; border-radius:20px; overflow:hidden; box-shadow:0 10px 30px rgba(15,23,42,0.08);">
            <div style="padding:28px 32px; background:linear-gradient(135deg, #0f172a, #1d4ed8); color:#fff;">
                ...
                <p style="margin:10px 0 0; font-size:15px; opacity:.92;">Reservation {{ $reservation->reservation_no }}</p>
            </div>
            ...
            <div style="padding:32px;">
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:28px 0; border-collapse:collapse; border:1px solid #e5e7eb; border-radius:14px; overflow:hidden;">
                    <tr>
                        <td style="padding:12px 16px; background:#f9fafb; font-weight:bold; width:40%;">Experiment</td>
                        <td style="padding:12px 16px;">{{ $reservation->experiment_title }}</td>
                    </tr>
                    <tr>
                        <td style="padding:12px 16px; background:#f9fafb; font-weight:bold; width:40%;">Laboratory</td>
                        <td style="padding:12px 16px;">{{ $reservation->laboratory?->laboratory_name ?? '—' }}</td>
                    </tr>
                    <tr>
                        <td style="padding:12px 16px; background:#f9fafb; font-weight:bold; width:40%;">Schedule</td>
                        <td style="padding:12px 16px;">{{ $reservation->reservation_date?->format('M d, Y') }} | {{ substr((string) $reservation->start_time, 0, 5) }} - {{ substr((string) $reservation->end_time, 0, 5) }}</td>
                    </tr>
                    <tr>
                        <td style="padding:12px 16px; background:#f9fafb; font-weight:bold; width:40%;">Status</td>
                        <td style="padding:12px 16px;">{{ $reservation->status }}</td>
                    </tr>
                </table>
